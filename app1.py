...
def main():
    st.set_page_config(page_title="Titanic Survive Prediction")  
    overview = st.container()
    left, right = st.columns(2)
    prediction = st.container()
    
    st.image("https://fajnepodroze.pl/wp-content/uploads/2020/05/Olympic_Titanic.jpg")

    with overview:
	    st.title("Titanic survive prediction")

    with left:
     sex_radio = st.radio( "Płeć", list(sex_d.keys()), format_func=lambda x : sex_d[x] )
     embarked_radio = st.radio( "Port zaokrętowania", list(embarked_d.keys()), index=2, format_func= lambda x: embarked_d[x] )
     pclass_radio = st.radio ("Numer klasy", list(pclass_d.keys()), format_func=lambda x : pclass_d[x] )
